...
- **Idle Time Threshold** - This setting decides how long a student can stay inactive before their session is marked as idle. The time is measured in minutes, with the default set to 30 minutes. For example, the system marks the session as idle if no action (like clicking or navigating) happens within 30 minutes. This helps educators distinguish between active participation and just being logged in.
