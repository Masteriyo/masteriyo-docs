...
#### Configuration
In the Payments submenu, click on **Taxes** to open tax configuration.
...
![Choose tax calculation method](https://raw.githubusercontent.com/Masteriyo/masteriyo-docs/main/assets/images/payments/select-tax-calculation-method.png)

**Option 1**: Prices Include Tax 
Course prices already include tax, as the tax amount is extracted from the total price. It’s useful when selling to a single tax jurisdiction because it’s simpler for fixed-rate scenarios.

**Option 2**: Calculate Tax at Checkout 
Tax is calculated during the checkout process, based on customer's billing address. It’s more accurate for international sales. 
...
![Display Tax Inclusive Price](https://raw.githubusercontent.com/Masteriyo/masteriyo-docs/main/assets/images/payments/display-tax-inclusive-price.png)

**Pro Tip: ** For better user experience, consider showing tax-inclusive prices to avoid checkout surprises. 
...
A popup will appear for tax configuration. Now, choose the country from the dropdown. 
Enter your tax percentage in the box. (e.g., 13 for 13%)
...
Uncheck **Apply single tax for entire country if you want to apply individual tax rate for each region of the country. 
After that you can pick the specific state or region:
 **Enter Tax Rate**: Input the tax percentage for the regions you select.
For non-selected states, you can set default tax rate. 

Click **"Add"** to save the regional tax settings 
