...
excerpt: Account page settings in Masteriyo — order history, invoice download, profile page, instructor apply button, plus a PHP API to add custom sidebar tabs.
---
...
- **Enable Quiz Attempts**: Show the Quiz Attempts tab on the Account page.
- **Enable Certificate**: Show the Certificates tab so students can download earned certificates.
- **Enable Subscriptions**: Show the Subscriptions tab.
- **Enable Zoom Session**: Show the Zoom Sessions tab.
- **Enable Google Meet**: Show the Google Meet sessions tab on the Account page.
- **Enable Certificate Page**: Show the Certificate Page tab on the Account page.
- **Enable Session Information**: Show the Session Information (calendar/session) tab on the Account page.
- **Show/Hide Header and Footer**: Choose whether the site header and footer are displayed on the Account page.
...
<Callout>**Caution:** Since the version that introduced `SidebarLinkRegistry`, the array passed to this filter is **keyed by link ID**, not numerically indexed. Callbacks that used `array_push()` or numeric keys should be updated to use associative keys. Prefer `SidebarLinkRegistry::add()` for new code; the filter is maintained for compatibility only.</Callout>
