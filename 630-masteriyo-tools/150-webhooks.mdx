...
## What are Webhooks?
Webhooks allow different applications to communicate automatically with each other in real-time. When an event occurs in one application, it can notify another application through webhooks, triggering specific actions or transferring data without manual intervention.
...
After filling in all the required information, you can click on **Create** to save the webhook.        
...
```
json{
  "event": "masteriyo_user_enrolled_to_course",
  "user_id": 123,
  "course_id": 456
}
```        
...
1. First, [enable Zapier addon](https://docs.masteriyo.com/premium-addons/zapier-integration) and configure.

2. In Zapier, create a new Zap and select "Webhooks by Zapier" as your trigger app.       
3. Choose "Catch Hook" as the event       
4. Copy the webhook URL provided by Zapier       
5. In Masteriyo, create a new webhook and paste the Zapier URL as the Delivery URL       
6. Select the events you want to trigger the webhook       
7. Configure the rest of your Zap to perform actions based on the Masteriyo data.       

## Using Zapier to Send Action Webhooks to Masteriyo

1. In Zapier, create a new Zap with your desired trigger       
2. Choose "Webhooks by Zapier" as your action app       
3. Select "POST" as the event       
