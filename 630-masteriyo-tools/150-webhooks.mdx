...
- New Reply to a question in a Course
...
Action webhooks allow external applications to perform actions within your Masteriyo LMS. This feature is available in Masteriyo Pro and enables you to integrate Masteriyo with CRM systems, automation tools, or other applications.
...
| `masteriyo_unenrolled_user_from_course` | Unenrolls a user from a course         | `course_id`, `user_id`                        |
| `masteriyo_course_user_completed`       | Marks a course as completed for a user | `user_id`, `course_id`                        |
...
1. First, [enable the Zapier addon](/premium-addons/zapier-integration).
...
![Choose Trigger Event](https://raw.githubusercontent.com/Masteriyo/masteriyo-docs/main/assets/images/webhooks/choose-trigger-event.png) 5. Under **Account**, click on **Sign In**.
...
9. Click on the **Continue** button to proceed further. It'll now provide you with the option to test the trigger. Run the test — if you see any records, the trigger has been set up correctly.
...
3. Configure the webhook:
   ...
   Payload Type: JSON
   Data: - event: `masteriyo_course_user_completed` - user_id: `[user ID number]` - course_id: `[course ID number]`
...
You can find the webhook token in your WordPress dashboard. Navigate to **Masteriyo > Webhooks** and click on the settings icon. Now, you'll see the webhook token in the side interface.
...
Copy this token and paste it under **Headers** of the Zap you're currently creating.
...
<Callout>
  In this documentation, we used **Student Enrolled** as the trigger that leads
  to the action of **Course Completion**. Similarly, you can use other triggers
  and set other actions.
</Callout>
