...
excerpt: Design course certificates with the new drag-and-drop Certificate Builder — pick a template, add text, logo, and student fields, then preview and export as PDF.
---
...
It works like a simple design tool — no code needed. This is different from the older [Classic Certificate Builder](/free-addons/certificate-builder).

<Callout>
  {" "}
  **Requirement:** The Certificate Builder add-on is free, and the new builder works on the free plugin — you can publish up to 2 certificates and use the standard fields and unlocked templates. [Masteriyo Pro](https://masteriyo.com/pricing/) removes the publish limit and unlocks the premium fields (Grade, Verification Code, Co-Instructors, QR Code) and the templates marked with a **Pro** badge.
</Callout>
...
| Requirement | Details                                                                                                                                      |
| ----------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| Masteriyo   | [Masteriyo](https://masteriyo.com/pricing/) — the new builder works in the free plugin (up to 2 published certificates). [Masteriyo Pro](https://masteriyo.com/pricing/) with an active license removes the publish limit and unlocks the premium fields (Grade, Verification Code, Co-Instructors, QR Code) and the locked premium templates. |
| Add-on      | [Activate the Certificate Builder add-on](/getting-started/activate-masteriyo-addons) from **Masteriyo > Add-ons** |
...
1. Click **Add New Certificate**.
   ...
   <Callout>
     {" "}
     **Note:** On Masteriyo Pro, an inactive or expired license opens a license popup
     here and blocks the next step — add or renew your license to continue. On the
     free plugin there is no license check; you are simply limited to 2 published
     certificates.
   </Callout>
...
<video width="100%" controls loop muted autoPlay playsInline>
  <source
    src="https://img.masteriyo.com/files/id:0f45e3ae7ef9238e01d5cc0dc7d623f7/directUpload/Builder_video_.webm"
    type="video/webm"
  />
</video>
...
| **Verification Code** <ProBadge size="xs" />           | A unique text code used to verify the certificate.                                            |
| **Grade** <ProBadge size="xs" />                       | The student's final grade. _Appears only if the Gradebook add-on is active._                  |
...
<Callout>
  {" "}
  **Note:** You only see the fields that apply to your setup. **Grade** needs
...
<Callout>
  {" "}
  **Note:** Certificates are generated fresh on each download — they are not stored
  per student. So if a course previously used an older Certificate (V1) and you switch
  it to a Certificate V2 here, every student (including those who finished earlier)
  gets the **latest** assigned certificate the next time they preview or download
  it.
</Callout>
...
- The **Classic Certificate Builder** (free) — the original certificate builder. See the [Classic Certificate Builder guide](/free-addons/certificate-builder).
- The **new Certificate Builder** (this guide) — the drag-and-drop designer, available on the free plugin with a 2-published-certificate limit; **[Masteriyo Pro](https://masteriyo.com/pricing/)** removes the limit and unlocks the premium fields and templates.
...
| Element — **Grade**                       | —           | ✓ (needs the **Gradebook** add-on)            |
...
<Callout>
  ...
  **Free version limit:** With the free plugin you can publish **up to 2** certificate
  ...
  the premium fields and templates.
</Callout>
...
You are running Masteriyo Pro and its license is inactive or expired. Add or renew your [Masteriyo Pro](https://masteriyo.com/pricing/) license to continue. On the free plugin there is no license check — you can create certificates but publish at most 2.
